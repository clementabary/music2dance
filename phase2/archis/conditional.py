...
class SequenceGenerator(nn.Module):
    def __init__(self, input_size, latent_size, size, output_size, n_blocks, n_cells=1):
        ...
        self.embed_label = nn.Embedding(4, 4)
        self.noise_gen = NoiseGen(input_size+4, latent_size, n_cells)
        self.decoder = FrameDecoder(latent_size, size, output_size, n_blocks)
        initialize_weights(self)

    def forward(self, x, labels):
        y = self.embed_label(labels).unsqueeze(1).expand(-1, x.size(1), -1)
        # x = nn.utils.rnn.pack_padded_sequence(x, lengths, batch_first=True)
        x = self.noise_gen(torch.cat((x, y), 2))
...
class SequenceDiscriminator(nn.Module):
    def __init__(self, channels_in, channels_h, seqlen, init_ker=7, n_blocks=1):
        ...
        self.embed_label = nn.Embedding(4, 4)
        self.conv1 = nn.Conv1d(channels_in+4, channels_h,
    ...
    def forward(self, x, labels):
        y = self.embed_label(labels).unsqueeze(-1).expand(-1, -1, x.size(-1))
        x = self.relu(self.conv1(torch.cat((x, y), 1)))
        x = self.blocks(x)
