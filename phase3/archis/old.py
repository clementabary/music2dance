...
class SequenceGenerator(nn.Module):
    def __init__(self, window_size, input_size, latent_size,
                 size, output_size, noise_size, n_blocks, n_cells=1, device="cpu"):
        ...
        self.device = device
        self.audio_enc = AudioEncoder(32, input_size)
...
class SequenceDiscriminator(nn.Module):
    def __init__(self, channels_in, channels_h, output_code, seqlen, device="cpu"):
...
class AudioDiscriminator(nn.Module):
    ...
    def forward(self, x):
        # FIXME: First experiments without last ReLU : mistake ??
...
class StickDiscriminator(nn.Module):
    def __init__(self, channels_in, channels_h, output_code, seqlen, init_ker=7, n_blocks=1):
        ...
        self.activation = nn.Tanh()  # FIXME: nn.Identity() before : mistake ??

    def forward(self, x):
        x = self.relu(self.conv1(x))
        x = self.blocks(x)
        x = self.activation(self.fconv(x))
        return x.squeeze(-1)
...
class AudioEncoder(nn.Module):
    # TODO: check if Tanh as last activation is relevant
    def __init__(self, f_maps, output_size):
        ...
        for _ in range(5):
            ...
            f_maps *= 2
        self.conv_layers.append(nn.Conv1d(1024, output_size, 2))
        self.activations.append(nn.Tanh())  # FIXME: nn.Identity() before : mistake ??
