...
import datetime
import numpy as np
from models.berkeley import SequenceGenerator, SequenceDiscriminator, AblatedSequenceDiscriminator
...
train_indices, val_indices, test_indices = indices[vsplit:], indices[tsplit:vsplit], indices[:tsplit]
        ...
        with torch.no_grad():
            for (real, _, audio, _, _) in val_loader:
                ...
                audio_slices = slice_audio_batch(audio, audio_feat_samples,
                                                 cutting_stride, pad_samples)
                audio_slices = audio_slices.to(device)
                real = real.view(real.size()[0], stick_length, output_size).permute(0, 2, 1)
                fake = gen(audio_slices, [stick_length] * real.size()[0])
                fake = fake.view(real.size()[0], stick_length, output_size).permute(0, 2, 1)
